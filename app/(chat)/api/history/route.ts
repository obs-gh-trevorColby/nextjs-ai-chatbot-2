...
export async function GET(request: NextRequest) {
  ...
  return tracer.startActiveSpan("history.get", async (span) => {
    try {
      ...
      span.setAttributes({
        "history.result_count": result.chats.length,
        "history.has_more": result.hasMore,
      });
      span.setStatus({ code: SpanStatusCode.OK });

      historyRequestCounter.add(1, { status: "success" });
      historyRequestDuration.record(Date.now() - startTime, {
        status: "success",
      });
      ...
      logger.emit({
        ...
        attributes: {
          ...
          result_count: result.chats.length,
