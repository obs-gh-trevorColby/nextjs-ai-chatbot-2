  ...
  return tracer.startActiveSpan("chat.post", async (span: Span) => {
    ...
    try {
      const {
        id,
        message,
        selectedChatModel: selectedChatModelFromBody,
      ...
      if (!selectedChatModel) {
        span.setStatus({
          code: SpanStatusCode.ERROR,
          message: "Missing chat model",
        });
        chatErrorCounter.add(1, { error_type: "missing_model" });

        logger.emit({
          severityNumber: SeverityNumber.ERROR,
          severityText: "ERROR",
          body: "Missing chat model in request",
      ...
      const userTypeFromSession: UserType = session.user.type;
      ...
      if (
        messageCount >
        entitlementsByUserType[userTypeFromSession].maxMessagesPerDay
      ) {
