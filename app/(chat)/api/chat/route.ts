  ...
  return tracer.startActiveSpan("chat.post", async (span) => {
    ...
    try {
      ...
      span.setAttributes({
        ...
        "chat.model": requestBody.model,
        "chat.message_count": requestBody.messages.length,
      });
      ...
      chatRequestCounter.add(1, { model: requestBody.model });
    } catch (error) {
      span.setStatus({
      ...
      logger.emit({
        ...
        severityText: "ERROR",
        body: "Invalid chat request body",
        attributes: { error: (error as Error).message },
      });

      chatErrorCounter.add(1, { error_type: "invalid_request" });
...
export function DELETE(request: Request) {
  return tracer.startActiveSpan("chat.delete", async (span) => {
    try {
      ...
      span.setAttributes({ "chat.id": id || "unknown" });
      ...
      if (chat?.userId !== session.user.id) {
        ...
        logger.emit({
          ...
          attributes: { chatId: id, userId: session.user.id },
        });
        return new ChatSDKError("forbidden:chat").toResponse();
      }
