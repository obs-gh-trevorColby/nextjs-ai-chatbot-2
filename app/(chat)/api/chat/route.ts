...
import { recordChatRequest, recordError, recordMessage } from "@/lib/metrics";
...
export async function POST(request: Request) {
  return tracer.startActiveSpan("chat.post", async (span) => {
    ...
    try {
      ...
      // Record metrics
      recordChatRequest(requestBody.selectedChatModel, "success", duration);
      recordMessage("user", requestBody.id);

      logger.emit({
        severityNumber: SeverityNumber.INFO,
        severityText: "INFO",
        body: "Chat response streaming started",
        attributes: {
          chatId: requestBody.id,
          duration,
        },
      ...
      recordError("chat_api_error", "/api/chat");
      if (requestBody?.selectedChatModel) {
        recordChatRequest(
